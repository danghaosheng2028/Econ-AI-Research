...
1.  **[sim_wealth.py](./sim_wealth.py)** - **财富流动与熵增模型**：模拟自由市场中的财富集中现象，探讨税收作为“负熵”对系统平衡的影响。
2.  **[sim_debt.py](./sim_debt.py)** - **债务违约与相变研究**：分析杠杆累积导致的系统性金融风险触发点。
3.  **[sim_market.py](./sim_market.py)** - **市场恐慌传染 (Ising Model)**：借鉴统计物理磁化模型，研究社交情绪如何驱动股市闪崩。
4.  **[sim_pricing.py](./sim_pricing.py) (重点推进)** - **AI 个性化定价博弈**：研究大数据算法对消费者剩余的提取效率及其伦理边界。
